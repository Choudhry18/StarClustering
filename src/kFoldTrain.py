...
if __name__ == '__main__':
    ...
    dataset = torch_du.TensorDataset(data, label)
    ...
    patience_counter = 0
    log = "reports/" + args.name + str(time.time()) + ".txt"
    initialize_log_file(log)
    k = 5
    ...
    for fold, (train_index, test_index) in enumerate(kf.split(data)):
        print(f'Fold {fold+1}')
        log_message(log, f'Fold {fold+1}')
        train_dataset = torch_du.Subset(dataset, train_index)
        test_dataset = torch_du.Subset(dataset, test_index)

        train_loader = torch_du.DataLoader(train_dataset, batch_size=64, shuffle=True)
        test_loader = torch_du.DataLoader(test_dataset, batch_size=64, shuffle=False)
